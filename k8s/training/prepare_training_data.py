...
def _load_classifier_rows(path: Path, source: str) -> list[dict[str, str]]:
        ...
        for row in reader:
            ...
            if text and tone in TONE_LABELS:
                ...
                if split not in {"train", "val", "test"}:
                    split = "train"
                rows.append({"text": text, "tone": tone, "split": split, "source": source})
    return rows


def _load_generator_examples(path: Path) -> list[dict[str, str]]:
...
def _build_classifier_rows(
    ...
    for row in feedback_rows:
        ...
        if tone and text:
            key = (text, tone)
            if key not in dedup:
                dedup[key] = {"text": text, "tone": tone, "split": "train", "source": "feedback"}
                added_feedback += 1
...
def _write_classifier_csv(path: Path, rows: list[dict[str, str]]) -> None:
    include_split = any(row.get("split") for row in rows)
    fieldnames = ["text", "tone", "split"] if include_split else ["text", "tone"]
    with path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for row in rows:
            payload = {"text": row["text"], "tone": row["tone"]}
            if include_split:
                payload["split"] = row.get("split", "train")
...
def main() -> None:
    ...
    batch_classifier_path = batch_root / "classifier_dataset.csv"
    if batch_classifier_path.exists():
        classifier_seed = _load_classifier_rows(batch_classifier_path, "batch_latest")
        classifier_seed_origin = str(batch_classifier_path)
    else:
    ...
    batch_generator_train_path = batch_root / "generator_train.jsonl"
    batch_generator_eval_path = batch_root / "generator_val.jsonl"
    if batch_generator_train_path.exists():
        generator_seed = _load_generator_examples(batch_generator_train_path)
        generator_seed_origin = str(batch_generator_train_path)
    else:
        generator_seed = _load_generator_examples((base / args.generator_seed_jsonl).resolve())
    ...
    training_manifest = {
        ...
        "artifacts": {
            "classifier_tone_csv": {
                ...
                "seed_origin": classifier_seed_origin,
            ...
            "generator_rewrites": {
                ...
                "seed_origin": generator_seed_origin,
                "eval_origin": generator_eval_origin,
                "stats": {
                    **generator_stats,
        ...
        "limitations": {
            "classifier": "Classifier retraining uses batch/latest when present, but tone balance still depends on the collected labeled corpus and feedback volume.",
            "generator": "Generator retraining uses batch/latest rewrites when present, but user-perceived improvement still depends on richer feedback and a faster serving path.",
        },
    }
